...
Créer un fichier .env et y coller ces variables d'environnement,
avec vos propres données concernant la DB;
...
<!-- Admin Name, under 20 chars, only letters (RegEx) ==> Ex: "loReMipSuMDOlorsiT" -->
ADMIN_PLAYER_NAME = ""
...
L'application BF Manager contient toutes les exigences demandées, plus quelques extra (pour le plaisir de coder);
 ...
 ==> Fonctionalités ajoutée en extra:
   ...
   <!-- Utilisation Chat -->
      Pour communiquer avec un autre joueur en privé, allez dans l'onglet "Social" et cliqué
      sur la bulle en face du nom du joueur;

      Attention ! ==> Si le joueur modifie son nom, il faudra recliquer sur la bulle du nouveau nom;
   

   <!-- passage en mode Admin -->
      Si vous avez correctement rentré un "Nom de joueur Admin" en variables d'environnement,
      celui ci doit être en dessous de 20 caractères et au dessus de 5, sans accents ni caractères spéciaux
